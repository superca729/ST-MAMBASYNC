...
from model.STMambaSync import STMambaSync
...
@torch.no_grad()
def test_model(model, testset_loader, predict_data_path, log=None):
    model.eval()
    ...
    save_true = os.path.join(predict_data_path, "y_true.npy")
    np.save(save_true, y_true)
    save_pred = os.path.join(predict_data_path, "y_pred.npy")
    np.save(save_pred, y_pred)
...
if __name__ == "__main__":
    ...
    model_name = STMambaSync.__name__

    with open(f"{model_name}.yaml", "r") as f:
        cfg = yaml.safe_load(f)
    cfg = cfg[dataset]

    model = STMambaSync(**cfg["model_args"])
    ...
    test_model(model, testset_loader, predict_data_path, log=log)
